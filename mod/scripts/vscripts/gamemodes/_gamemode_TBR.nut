...
bool ForceStartGame = false
bool ForceEndGame = false
bool PBRGamePlaying = false
...
void function GamemodeTBR_Init()
{
    ...
    AddCallback_GameStateEnter( eGameState.Prematch, PBRIntroStart )
    AddCallback_GameStateEnter( eGameState.Playing, PBRStartPlaying )
    AddCallback_OnClientConnected( PBROnClientConnect )
    AddCallback_OnClientDisconnected( PBROnClientDisconnect )
    AddCallback_OnPlayerKilled( OnPlayerKilled )

    minPlayer = GetCurrentPlaylistVarInt("min_players", 2)
    WaitingPlayerTime = GetCurrentPlaylistVarInt("waitingPlayerTime", 30)

    PBRIntroLength = GetCurrentPlaylistVarFloat("TBR_IntroLength", 0)
    ...
    ClassicMP_SetCustomIntro( PBRIntroSetup, PBRIntroLength )
    #endif
}

void function PBRIntroSetup()
{   
...
void function WaitForPlayers( )
{
	...
	float endTime = Time() + WaitingPlayerTime
    foreach( entity player in GetPlayerArray() ) {
        Remote_CallFunction_NonReplay(player, "Cl_ConnectedPlayer", GetPlayerArray().len(), minPlayer )
    }
	while ( ( endTime > Time() ) && GetPlayerArray().len() < minPlayer && !ForceStartGame) {
        foreach(entity player in GetPlayerArray()) {
            Remote_CallFunction_NonReplay(player, "Cl_OnWaitingPlayer", WaitingPlayerTime)
        }
        wait 1.0
        WaitingPlayerTime--
        //WaitFrame()
...
void function PBRIntroStart()
{   
    printt( "Prematch Start" )
    thread PBRIntroStartThreaded()
    foreach ( entity player in GetPlayerArray() ) {
        PBROnClientConnect( player )
        if(GetPlayerArray().len() >= minPlayer) {
...
void function PBRIntroStartThreaded()
{
...
void function PBRStartPlaying() {
    printt( "Playing Start" )
    PBRGamePlaying = true
    NumAlivePlayer = GetPlayerArray().len()
    AtStartNumAlivePlayer = NumAlivePlayer
    foreach ( entity player in GetPlayerArray() ) {
...
//Only used for dev, use !feg (force end game) in chat to force the end of the game
// What the fuck I did here ??????
void function PBRPlaying() {
    while(PBRGamePlaying) {
        if( ForceEndGame ) {
            PBRGamePlaying = false
        }
        wait 0.1
    }
    foreach(entity player in GetPlayerArray()) {
...
void function PBROnClientConnect( entity player )
{   
...
ClServer_MessageStruct function MyChatCommande(ClServer_MessageStruct message)
{
    if (message.message.find("!fsg") != null)
    {
        ForceStartGame = true
        Chat_ServerBroadcast("Force start game.")
    } else if (message.message.find("!feg") != null)
    {
        ForceEndGame = true
